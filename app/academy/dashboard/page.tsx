...
export default function AcademyDashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Load all students of academy members
      const memberIds = (mems || []).map(m => m.user_id);
      if (memberIds.length > 0) {
        const { data: sts } = await supabase
          .from("students")
          .select("*")
          .in("tutor_id", memberIds);
        setStudents(sts || []);
    ...
    <div style={{ minHeight: "100vh", background: "#F8FAFF", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <nav style={{ padding: "16px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", background: "white", borderBottom: "1px solid #E2E8F0", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: "#94A3B8" }}>Academy {role === "admin" ? "Admin" : "Member"}</p>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          ...
          {/* Members List */}
          <div style={{ background: "white", borderRadius: 16, padding: "24px 28px", border: "1px solid #E2E8F0", boxShadow: "0 1px 4px rgba(0,0,0,0.05)", gridColumn: role === "admin" ? "auto" : "1 / -1" }}>
                ...
                return (
                  <div key={member.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: "#F8FAFC", borderRadius: 12, border: "1px solid #E2E8F0" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      {role === "admin" && !isAdmin && (
                        <button onClick={() => removeMember(member.id, member.user_id)} disabled={removing === member.id}
